...
import { createRoomAction } from '@/lib/impostor-actions';

export function ImpostorsRiddleHomePage() {
  ...
  const handleJoinRoom = (e: React.FormEvent) => {
    ...
    if (roomCode.trim().length === 4) {
      setIsJoining(true);
      router.push(`/impostors-riddle/${roomCode.trim().toUpperCase()}`);
    }
  };
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      router.push(`/impostors-riddle/${newRoomCode}`);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center text-center">
      ...
      <h1 className="mt-4 text-4xl font-extrabold tracking-tighter sm:text-5xl md:text-6xl font-headline">
        The Impostor's Riddle
      </h1>
      <p className="mt-4 max-w-md text-muted-foreground">
        A game of deduction and deception. Find the impostor.
      </p>
      <Card className="mt-8 w-full max-w-sm shadow-2xl">
        <CardHeader>
          <CardTitle>Join the Investigation</CardTitle>
          <CardDescription>
