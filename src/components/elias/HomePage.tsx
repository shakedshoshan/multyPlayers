...
import { createEliasRoomAction } from '@/lib/elias-actions';

export function EliasHomePage() {
  ...
  const handleJoinRoom = (e: React.FormEvent) => {
    ...
    if (roomCode.trim().length === 4) {
      setIsJoining(true);
      router.push(`/elias/${roomCode.trim().toUpperCase()}`);
    }
  };
  ...
  const handleCreateRoom = async () => {
    setIsCreating(true);
    try {
      const newRoomCode = await createEliasRoomAction();
      router.push(`/elias/${newRoomCode}`);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center text-center">
      ...
      <p className="mt-4 max-w-md text-muted-foreground">
        A fast-paced word-guessing game for teams.
      </p>
      <Card className="mt-8 w-full max-w-sm shadow-2xl">
        <CardHeader>
          <CardTitle>Let's Play Elias</CardTitle>
          <CardDescription>
